...
| Cron 任务 | ✔（定时 Agent 使用同一套已配置备用链） |
| 辅助任务（视觉、压缩等） | ✘（使用各自的提供商链——见下文） |

Cron 任务会解析与交互式会话相同的顶层 `fallback_providers` /
`fallback_model` 链。如果主提供商在定时 Agent 启动前无法完成认证，
cron 会解析第一个可用的备用提供商，并使用该备用条目的模型运行本次
任务。如果提供商在 Agent 已运行后失败，则继续使用常规的单轮次备用路径。

...
| `TAVILY_API_KEY` | Tavily API key for AI-native web search, extract, and crawl; may contain one key or a comma/newline/semicolon-separated key list ([app.tavily.com](https://app.tavily.com/home)) |
| `TAVILY_API_KEYS` | Optional explicit Tavily multi-key pool. Accepts comma/newline/semicolon-separated values or a JSON array; keys are used fill-first with cooldown failover. |
| `TAVILY_KEY_COOLDOWN_SECONDS` | Optional cooldown for Tavily quota/auth failures before a key re-enters the pool. Defaults to 172800 seconds (2 days). |
